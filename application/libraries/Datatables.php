<?php
	...
	class Datatables {
		...
		public function add_column($column, $content = Null, $match_replacement = null) {
		...
		private function get_ordering() {
			...
			if ($this->ci->input->post('order')) {
				...
				foreach ($this->ci->input->post('order') as $key) {
					$column = $data[$key['column']]['data'];
					if($column == 'index_column' OR $column == ''){
						$this->ci->db->order_by($this->columns[0], $key['dir']);
					}else{
					if ($this->check_cType()) {
